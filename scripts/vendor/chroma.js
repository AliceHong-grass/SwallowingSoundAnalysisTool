(function() {
  ...
  unpack = function(args) {
    if (args.length >= 3) { return [].slice.call(args); }
    else { return args[0]; }
  };
  ...
  chroma = function() {
    if (arguments[0] instanceof Color) { return arguments[0]; }
  ...
  Color = (function() {
    function Color() {
      ...
      if (_input[mode] != null) {
        ...
        if (mode) me._rgb = clip_rgb(_input[mode].apply(_input, args));
      }
      if (me._rgb == null) console.warn('unknown format: ' + args);
      if (me._rgb == null) me._rgb = [0, 0, 0];
    ...
    Color.prototype.toString = function() { return this.name(); };
  ...
  bezier = function(colors) {
    ...
    if (colors.length === 2) {
      ...
      I = function(t) {
        if (t < 0.5) { return I0(t * 2); }
        else { return I1((t - 0.5) * 2); }
      };
    }
    return I;
  };

  chroma.bezier = function(colors) { return chroma.scale(bezier(colors)); };

  chroma.cubehelix = function(start, rotations, hue, gamma, lightness) {
    var dh, dl;
    if (start == null) start = 300;
    if (rotations == null) rotations = -1.5;
    if (hue == null) hue = 1;
    if (gamma == null) gamma = 1;
    if (lightness == null) lightness = [0, 1];
  ...
  chroma.random = function() {
    ...
    code = '#';
    for (i = o = 0; o < 6; i = ++o) code += digits.charAt(floor(Math.random() * 16));
  ...
  Color.prototype.rgb = function() { return this._rgb.slice(0, 3); };

  Color.prototype.rgba = function() { return this._rgb; };
  ...
  _guess_formats.push({
    ...
    test: function(n) {
      ...
      if (a.length === 4 && type(a[3]) === "number" && a[3] >= 0 && a[3] <= 1) { return 'rgb'; }
    }
  });
